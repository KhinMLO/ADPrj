...
namespace LogicUniversity.Control
{
    public class DisbursementController
    {
                ...
                foreach (Model.DisbursementItemViewModel item in listToUpdate)
                {
                    if (disItem.DisbursementItemID == item.DisbursementItemID)
                    {
                        ...
                        if (finalQuantity >= 0)
                        {
                            ...
                            {
                                emailCrt.sendEmailToRepresentativeToCollect(disItem.Disbursement.DepartmentID,disItem.DisbursementID.GetValueOrDefault().ToString());
            ...
            foreach (DisbursementItem disItem in olddisItemList)
            {
                ...
                foreach (Disbursement dis in disList)
                {
                    if (dis.DepartmentID.Equals(disItem.Disbursement.DepartmentID)) { 
                        ...
                        if (flag == 0)
                        {
                            ...
                            List<RequisitionItem> reqList = disItem.RequisitionItems.ToList();
                            temp_disburItem.RequestDate = reqList[0].Requisition.Date;
                            temp_disburItem.Status = "Not Allocated";
                            foreach (RequisitionItem reqItem in reqList) {
                                temp_disburItem.RequisitionItems.Add(reqItem);
                            }
                            temp_disburItem.DisbursementID = dis.DisbursementID;
                ...
                if (flag == 0)
                {
                    ...
                    if (disList.Count == 0)
                    {
                        ...
                        ctx.SaveChanges();
                        temp_disburItem.DisbursementID = temp_disbur.DisbursementID;
                        temp_disbur.DisbursementItems.Add(temp_disburItem);
                        ctx.SaveChanges();
                        disList = ctx.Disbursements.Where(x => (x.status == "inProgress" || x.status == "EmailSent") && x.CollectionDate >= DateTime.Today).ToList();
                    }
                    disList.Add(temp_disbur);
                    ctx.SaveChanges();
                }
            }
        }
        ...
        public List<NotCollectedModel> getToNotCollectedList()
        {
            ...
            List<Disbursement> disList = ctx.Disbursements.Where(x => (x.status == "inProgress" || x.status == "EmailSent ") && x.CollectionDate <= DateTime.Today).ToList();
        ...
        public string changeToNotCollected(int disbursementID)
        {
            ...
            ctx.SaveChanges();
            List<DisbursementItem> disItemList = dis.DisbursementItems.ToList();
            foreach(DisbursementItem disItem in disItemList){
                disItem.Item.Quantity += disItem.Quantity.GetValueOrDefault() - disItem.RemainingQty.GetValueOrDefault();
                disItem.RemainingQty = 0;
            }
            ctx.SaveChanges();
            return "success";
