...
namespace LogicUniversity.Control
{
    ...
    public class RequisitionApprovalControl
    {
            ...
            foreach(Model.Requisition req in temp)
            {
                foreach(Model.RequisitionItem reqItem in req.RequisitionItems)
                {
                    if(reqItem.Status.Equals("PendingApproval"))
                        result.Add(new Model.RequisitionApproval(reqItem.RequisitionID ?? default(int), reqItem.RequisitionItemID,req.Employee.Name,req.Date.GetValueOrDefault().ToString("dd/MM/yyyy").ToString(),reqItem.Item.Description, reqItem.Quantity.ToString()));
                }
            }
            return result;
        }
        ...
        public string ApproveRequisition(List<Model.RequisitionApproval> reqlist)
        {
            ...
            string result = emailCrt.sendForReqReviewed(requisitionItemList);
            return "success";
        }        
    }
}